...
- **1シート＝1日×1つの箱**。箱の種類（上銅 / 銅ダライ / 銅スクラップ）はブック単位で指定し、
  同じ日のシートは**1枚の記録票**にまとまります（アプリの記録票は日付×工場で1枚のため）
- 列の位置はブックによって違う（「累積差」列がある版・無い版）ため、**見出しの文言から列を探します**
- **日付はシート名**（例「上銅7.23」）から決めます。日付セルは前日のシートを複製したまま直っていないことが
  多いため、食い違う場合は警告に出したうえでシート名を採用します。年はシート名に無いので画面で指定します
  （ブックの途中で月が戻れば翌年扱い）
- **重量 = 投入重量 − 箱重量**（サーバー側で計算し直します）。「実投入」列が手入力で上書きされていて
  食い違う行は警告に出ます。累積（投入前/投入後）はExcelの「累積」列と同じく箱ごとの積み上げで入ります
- **回収箱測定値**は箱ごとの値なので、その日の箱の合計を入れます。1つでも未記入の箱があるとその日は空にします
  （部分的な合計で差異率を出さないため）
